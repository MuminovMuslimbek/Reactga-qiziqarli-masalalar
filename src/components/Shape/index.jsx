...
const Index = () => {
    ...
    return (
        <div className="bg-white flex flex-col gap-[10px] p-[20px] rounded-[8px] max-w-[500px] w-full">
            ...
            <input onChange={(e) => setInput(e.target.value)} id="shape" type="number" placeholder="Enter.." className="p-[8px] border border-[#0a1050] rounded-[8px]"
            />
            <button
                onClick={() => {
                    if (input) {
            ...
            <textarea value={result} id="resultShape" className="p-[8px] h-full border resize-none border-[#0a1050] rounded-[5px] outline-none" readOnly
            ></textarea>
        </div>
    );
};
